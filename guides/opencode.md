...
OpenCode 是一款在终端中运行的编码代理，适合在命令行中完成开发任务。接入 {{SITE_NAME}} 时，供应商配置使用 `@ai-sdk/anthropic`，接入地址（Base URL）带 `/v1`
...
填入以下配置，并将 `models` 与 `model` 中的模型 ID 替换为 **模型广场** 当前可用的 Claude 模型 ID，保持两处 ID 一致：
...
示例仅保留一个模型条目，用于说明配置结构。使用前核对 **模型广场** 当前可用的模型 ID、能力和计费信息，并据此调整配置，不要继续使用已下线的模型 ID

## 登录与启用

保存 `opencode.json` 后，在终端中运行 `opencode auth login`，按提示选择 **Other**，将 **Provider ID** 填为 `codeflow`，再输入令牌。令牌由 OpenCode 凭据库管理，不写入 JSON

## 验证接入

在终端中运行 `opencode` 进入界面，输入 `/models`，确认列表中显示已配置的模型。选择该模型并发送一条测试消息，收到正常响应，即表示接入成功
