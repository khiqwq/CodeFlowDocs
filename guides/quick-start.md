...
> **配置说明**
>
> 上图为 2026 年 9 月 6 日的界面留档。换算比例、优惠、最低金额和余额有效期均可能调整，实际付款金额与到账额度以 **账户充值** 页面实时显示为准

除充值外，也可以在 **订阅套餐** 中购买套餐。两种计费方式的差异与预扣规则见[模型与计费](/guides/models-and-billing.md)

## 创建令牌

进入 **令牌管理 → 创建令牌**，填写 **名称**、选择 **分组**，按需设置 **费用限制**。完整令牌只在创建时显示一次，创建后立即复制并妥善保存
...
安装完成后运行 `codeflow`，按提示登录；在 **令牌管理** 中创建令牌，选择 **写入配置**，再选择要配置的客户端。写入前工具会显示将要修改的文件。详细步骤见[一键配置客户端](/guides/codeflow-cli.md#一键配置客户端)
...
|客户端无法连接|按对应客户端页面核对接入地址，确认末尾是否带 `/v1`|
|充值后未到账|刷新 **账户充值** 页面核对记录，仍未到账时携带订单号提交工单|
