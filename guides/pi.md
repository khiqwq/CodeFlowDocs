...
填入以下配置，并将 `apiKey` 的值 `sk-您的令牌` 替换为在 {{SITE_NAME}} 创建的令牌：
...
发送一条测试消息，例如“请回复：连接成功”。收到正常响应，即表示接入成功；在 {{SITE_NAME}} 的 **使用日志** 中确认对应调用，核对模型与用量
...
|安装后找不到 `pi`|重新打开终端，确认 npm 全局命令目录已加入 `PATH`|
...
- [Pi 自定义模型配置](https://pi.dev/docs/latest/models)
- [Pi 技能文档](https://pi.dev/docs/latest/skills)
- [Pi v0.85.1 使用文档（核验版本）](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/README.md)
- [Pi Agent 与 OMP 快速上手指南（掘金原文）](https://juejin.cn/post/7649955227479932991)
